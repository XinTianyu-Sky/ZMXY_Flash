package config
{
   ...
   import com.multi4399.keys.NK;
   import com.multi4399.keys.UK;
   import event.AEventDispatcher;
   import export.FloorBg;
   import export.GameInfo;
   import export.Infomation;
   import export.hero.Role4;
   import export.muti.NodeInfo;
   ...
   import my.AllEquipment;
   import my.KeyBoardControl;
   import my.MyEquipObj;
   import my.MyProtectedProperty;
   import my.ViewControllor;
   import petInfo.PetInfo;
   import storage.MemoryClass;
   ...
   public class Config
   {
      ...
      public var openinghhe:Boolean = true;
      ...
      public var openPig:Boolean = false;
      
      public var isHideDebug:Boolean = false;
      
      public var isFirstPlayGame:Boolean = true;
      
      public var isinthestrengthpack:Boolean = false;
      
      public var isintheselectmap:Boolean = false;
      
      public var isingamemenu:Boolean = false;
      
      public var isingameshop:Boolean = false;
      ...
      public var isManyOpen:Boolean = false;
      
      public var afterManyOpenCheckFunc:Function = null;
      
      public var server:Client;
      
      public var isServerOk:Boolean = false;
      ...
      public var isRoomOnwer:Boolean = false;
      ...
      public function getHeroBySidAndRoleId(param1:uint, param2:uint) : BaseHero
      {
         ...
         while(_loc4_ < this.pWorld.getOtherHeroArray().length)
         {
            ...
            if(_loc3_ && _loc3_.sid == param1 && _loc3_.getRoleId() == param2)
            {
               return _loc3_;
            }
            _loc4_++;
         ...
         if(param1 == this.sid)
         {
            if(this.hero1 && this.hero1.getRoleId() == param2)
            {
               return this.hero1;
            }
            if(this.hero2 && this.hero2.getRoleId() == param2)
            {
               return this.hero2;
            }
         }
      ...
      public function getPetBySidAndRoleId(param1:uint, param2:uint) : BasePet
      {
         ...
         var _loc4_:BaseHero;
         if(_loc4_ = this.getHeroBySidAndRoleId(param1,param2))
         {
            return _loc4_.getPet();
         }
         return null;
      }
      ...
      public function sendSelfMutiUserInfo(param1:uint) : void
      {
         ...
         if(this.server && this.isServerOk)
         {
      ...
      public function getUserInfoByUid(param1:uint) : void
      {
         if(this.server && this.isServerOk)
         {
            this.server.ins_get_user(UK.ID | UK.INDEX | UK.DATA,param1);
      ...
      public function getOtherPlayersInfo() : *
      {
         if(this.server && this.isServerOk)
         {
            ...
            this.server.ins_get_users(UK.INDEX | UK.DATA,new ByteArray());
      ...
      public function createRoom(param1:Array, param2:uint) : void
      {
         ...
         if(this.server && this.isServerOk)
         {
            this.nodeInfo.GameMode = param1[1];
            this.nodeInfo.averLevel = this.getAverageLevel();
            this.nodeInfo.roomName = param1[0];
            this.nodeInfo.mpInfo = param1[2];
            _loc3_ = this.getAverageLevel();
            this.nodeInfo.averLevel = _loc3_;
      ...
      public function getSaveInfo() : XML
      {
         ...
         var _loc2_:* = "<heros>";
         if(this.hero1 && this.hero1.getPlayer())
         {
            _loc2_ += "<heros>";
            if(this.logInfo && this.logInfo.name)
            {
         ...
         if(this.hero2 && this.hero2.getPlayer())
         {
            _loc2_ += "<heros>";
            if(this.logInfo && this.logInfo.name)
            {
      ...
      public function getAverageLevel() : uint
      {
         ...
         var _loc3_:uint = 0;
         var _loc1_:uint = this.getPlayerArray().length;
         var _loc4_:uint = 0;
         while(_loc4_ < _loc1_)
         {
            _loc2_ = this.getPlayerArray()[_loc4_] as BaseHero;
            _loc3_ += _loc2_.roleProperies.getLevel();
            _loc4_++;
         }
         return uint(_loc3_ / _loc1_);
      }
      ...
      {
         if(this.hero1 && this.gameSence.contains(this.hero1))
         {
            if(this.hero1.getPet() && this.gameSence.contains(this.hero1.getPet()))
            {
               this.gameSence.removeChild(this.hero1.getPet());
            }
            this.gameSence.removeChild(this.hero1);
         }
         if(this.hero2 && this.gameSence.contains(this.hero2))
         {
            if(this.hero2.getPet() && this.gameSence.contains(this.hero2.getPet()))
            {
      ...
      {
         var _loc1_:Array = [];
         if(this.hero1 && !this.hero1.isDead())
         {
            _loc1_.push(this.hero1);
         }
         if(this.hero2 && !this.hero2.isDead())
         {
      ...
      public function putQHsInArray(param1:String, param2:Array = null, param3:int = 1) : Array
      {
         ...
         else
         {
            ...
            (_loc4_ = this.allEquip.findByName(param1)).setnum(param3);
            param2.push(_loc4_);
         }
         return param2;
      }
      ...
      public function addActionMask(param1:String = "") : void
      {
         ...
         var _loc2_:Sprite = this.stage.getChildByName("execuitionAction" + param1) as Sprite;
         if(_loc2_ == null)
         {
            _loc3_ = AUtils.getNewObj("mask" + param1) as Sprite;
            _loc3_.name = "execuitionAction" + param1;
            this.stage.addChild(_loc3_);
         }
         else
         {
            trace("The Action Mask has already excist");
         }
      }
      ...
      private function completeHandler(param1:*) : void
      {
         ...
         var _loc2_:* = com.adobe.serialization.json.JSON.decode(this.urlLoader.data);
         var _loc3_:Array = String(_loc2_["time"]).split(" ");
         this.curdate = _loc3_[0];
      }
   }
}
